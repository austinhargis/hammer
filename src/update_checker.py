...
class UpdateChecker(tk.Toplevel):
    ...
    def __init__(self, parent):
        ...
        auto_update_frame.pack(fill='both', expand=True, padx=parent.padding * 2, pady=parent.padding)
        tk.Label(auto_update_frame, text=f'Automatic Updates {parent.save_m.data["automatic_updates"]}').pack()

        subtitle_frame = tk.Frame(self)
        subtitle_frame.pack(fill='both', expand=True, padx=parent.padding*2, pady=parent.padding)
        tk.Label(subtitle_frame, text=f'Last Checked {parent.save_m.data["last_update_check"]}').pack()
